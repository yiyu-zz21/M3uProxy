...
1. **基础镜像结构**
   - `m3u_proxy/Dockerfile`：基于 `rust:1.75` 的多阶段构建，编译 release 二进制后落地到 `debian:bookworm-slim`，并在 `/app` 目录下加载 `Gather.m3u`。
   - `m3u_proxy_frontend/Dockerfile`：使用 `node:20` 构建 Vite 产物，再由 `nginx:alpine` 提供静态托管，`nginx.conf` 对所有路径回退到 `index.html` 以适配前端路由。

2. **docker-compose 一键启动**
   ```bash
   docker compose up --build
   ```
   - `backend` 服务：监听 `8006`，自动挂载宿主机的 `./m3u_proxy/Gather.m3u`，方便后续更新频道列表。
   - `frontend` 服务：监听 `5173`（Nginx 对外 80 端口映射），在构建阶段注入 `VITE_API_BASE_URL=http://backend:8006`，容器间通过 Compose 网络通信。

3. **目录与相对路径注意事项**
   - `docker-compose.yml` 位于仓库根目录，所有 `context` 与 `volumes` 均使用 `./m3u_proxy`、`./m3u_proxy_frontend` 等相对路径，在任何工作目录执行都能保持一致。
   - 若需替换新的 m3u 文件，直接更新 `m3u_proxy/Gather.m3u`，重启容器即可生效（挂载为只读，确保容器内部不可写）。

4. **自定义 API 地址**
   - 默认情况下前端会回退到当前域名访问后端，当以容器方式部署时，可通过 `docker-compose.yml` 的 `build.args.VITE_API_BASE_URL` 覆盖，或在本地构建前设置环境变量 `VITE_API_BASE_URL`。
